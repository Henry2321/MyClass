...
router.post("/join", auth, async (req, res) => {
  try {
    ...
    const classToJoin = await Class.findOne({ code }).populate(
      "teacher",
      "name",
    );
    if (!classToJoin) {
    ...
    await createNotification(
      classToJoin.teacher._id,
      req.user._id,
      "Sinh viên mới tham gia lớp",
      `${req.user.name} đã tham gia lớp "${classToJoin.name}"`,
      "class",
      { classId: classToJoin._id },
    );
...
router.post("/:id/import-students", auth, teacherAuth, async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Không có quyền truy cập lớp này" });
    }
    ...
    for (const s of students) {
      ...
      if (!user) {
        ...
        if (
          user.name.startsWith("Sinh viên ") &&
          s.name &&
          !s.name.startsWith("Sinh viên ")
        ) {
          user.name = s.name;
          await user.save();
        }
