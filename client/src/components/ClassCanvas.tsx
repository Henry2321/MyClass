...
export default function ClassCanvas({ classId }: ClassCanvasProps) {
  ...
  const toggleAttendancePanel = () => {
    setShowAttendancePanel(prev => !prev)
  }
    ...
    input.onchange = async (e: any) => {
      ...
      reader.onload = async (event) => {
        ...
        try {
          ...
          const res = await fetch(getApiUrl(`/api/classes/${classId}/import-students`), {
            method: 'POST',
            headers: {
  ...
  useEffect(() => {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7887/ingest/e94691c6-ad0a-42cd-9247-f9986cc7c541',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'687afc'},body:JSON.stringify({sessionId:'687afc',runId:'pre-debug',hypothesisId:'H5',location:'client/src/components/ClassCanvas.tsx:728',message:'emit join_classroom',data:{isJoined,hasSocket:!!socket,socketId:socket?.id||null,classId:classId},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
    socket.emit('join_classroom', {
      classId: classId,
      user: {
        dbId: user?.id || user?._id, // Gửi ID thực từ Database
        mssv: studentId,             // Gửi MSSV nhập từ ô input
        name: userName,
        avatar: user?.role === 'teacher' ? teacherCharacter : studentCharacters[selectedCharIndex],
    ...
    function update(){
      ...
      if (keyboardMoveX !== 0 || keyboardMoveY !== 0) {
        ...
        if (distance <= spd + 1) { // Thêm 1px buffer để snap mượt hơn
            ...
            if (index >= 0 && index < mapData.width * mapData.height) {
              for (const layer of mapData.layers) {
                ...
                if (name === "Class" || name === "cảnh vật") {
                  ...
                  if ((tile >= 4423 && tile <= 4457) || (tile >= 2977 && tile <= 2995) || (tile >= 5213 && tile <= 5247)) {
                    canMoveX = false
                    break
                  }
                }
  ...
  }, [isJoined, userName, selectedCharIndex, socket, studentId, user?.role, classId])
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                ...
                {user?.role === 'student' && (
                  <div style={{ display: "flex", justifyContent: "center", gap: "30px" }}>
                    <button 
                      onClick={() => setSelectedCharIndex((prev) => (prev - 1 + studentCharacters.length) % studentCharacters.length)}
                      style={{ 
                        background: "none", 
                    ...
                    <button 
                      onClick={() => setSelectedCharIndex((prev) => (prev + 1) % studentCharacters.length)}
                      style={{ 
                        background: "none", 
                        border: "1px solid #4b4c56", 
                        color: "white", 
            ...
            <button 
              onClick={async () => {
                ...
                if (hasName && (isTeacher || hasId)) {
                  ...
                  alert(isTeacher ? "Vui lòng nhập Tên!" : "Vui lòng nhập đầy đủ Tên và MSSV!");
                }
              }}
              style={{
        ...
        <div style={{
          ...
          {user?.role === 'teacher' && (
            <div style={{ 
              ...
              >
                <span style={{ fontSize: '16px' }}>{showAttendancePanel ? "✖" : "📊"}</span> 
          ...
          {showAttendancePanel && user?.role === 'teacher' && (
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '10px' }}>
                <span style={{ fontWeight: 'bold', fontSize: '15px', color: '#4ade80' }}>📋 Điểm danh hôm nay</span>
              ...
              <div className="attendance-table" style={{ overflowY: 'auto', flex: 1 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                  <thead>
                    <tr style={{ textAlign: 'left', color: '#64748b', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                      ...
                      <th style={{ padding: '8px 4px', textAlign: 'center' }}>Check</th>
                      <th style={{ padding: '8px 4px', textAlign: 'center' }}>Out</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      attendanceList.map((s, idx) => (
                        <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', transition: 'background 0.2s' }}>
                          <td style={{ padding: '10px 4px' }}>
                            ...
                            <div style={{ fontSize: '11px', color: '#64748b' }}>{s.mssv}</div>
                          </td>
                          <td style={{ padding: '10px 4px', textAlign: 'center', fontSize: '16px' }}>
                            {s.isPresent ? "✅" : "❌"}
                          </td>
                          <td style={{ padding: '10px 4px', textAlign: 'center' }}>
                            <span style={{ 
                              padding: '2px 8px', 
                              backgroundColor: s.leaveCount > 2 ? 'rgba(239, 68, 68, 0.15)' : 'rgba(255,255,255,0.03)',
