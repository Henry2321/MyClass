  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        const res = await fetch(getApiUrl(`/api/students/class/${classData._id}`), {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content large" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="class-detail-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '20px' }}>
            <div className="class-detail-info" style={{ padding: '20px', backgroundColor: '#f8fafc', borderRadius: '12px' }}>
              <h4 style={{ marginBottom: '15px', color: '#1e293b' }}>Thông tin chi tiết</h4>
              <p style={{ marginBottom: '10px' }}><strong>Mã lớp:</strong> <span style={{ color: '#4ade80', fontWeight: 'bold' }}>{classData.code}</span></p>
              <p style={{ marginBottom: '10px' }}><strong>Giáo viên:</strong> 👨🏫 {classData.teacher.name}</p>
              <p style={{ marginBottom: '10px' }}><strong>Số sinh viên:</strong> 👥 {students.length}</p>
              <p style={{ marginBottom: '10px' }}><strong>Mô tả:</strong> {classData.description || 'Chưa có mô tả'}</p>
              <p><strong>Ngày tạo:</strong> 📅 {new Date(classData.createdAt).toLocaleDateString('vi-VN')}</p>
            </div>
            
            <div className="class-students-list" style={{ padding: '20px', backgroundColor: '#f8fafc', borderRadius: '12px' }}>
              <h4 style={{ marginBottom: '15px', color: '#1e293b' }}>Danh sách sinh viên trong lớp</h4>
              {loading ? (
                <p>Đang tải danh sách...</p>
              ) : students.length === 0 ? (
                <p style={{ color: '#64748b', fontStyle: 'italic' }}>Chưa có sinh viên nào trong lớp này. Vui lòng sử dụng tính năng "Quản lý" để Import danh sách Excel.</p>
              ) : (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ borderBottom: '2px solid #e2e8f0', textAlign: 'left' }}>
                        ...
                        <th style={{ padding: '10px', textAlign: 'center' }}>Điểm danh</th>
                        <th style={{ padding: '10px', textAlign: 'center' }}>Số lần thoát</th>
                      </tr>
                    </thead>
                    <tbody>
                      {students.map((student, index) => (
                        <tr key={index} style={{ borderBottom: '1px solid #e2e8f0' }}>
                          ...
                          <td style={{ padding: '10px', textAlign: 'center' }}>
                            <span style={{ 
                              padding: '2px 8px', 
                              backgroundColor: student.leaveCount > 2 ? '#fee2e2' : '#f1f5f9',
                              color: student.leaveCount > 2 ? '#ef4444' : '#64748b',
                              borderRadius: '12px',
                              fontSize: '12px',
                              fontWeight: 'bold'
                            }}>
      ...
      reader.onload = async (event) => {
        ...
        try {
          if (fileName.endsWith('.xlsx') || fileName.endsWith('.xls')) {
            ...
            jsonData.forEach(row => {
              if (Array.isArray(row) && row.length >= 2) {
                ...
                row.forEach(cell => {
                  ...
                  else if (cellStr.length > 2 && /[a-zA-ZÀ-ỹ]/.test(cellStr) && !cellStr.includes('@')) {
                    name = cellStr;
                  }
                });
            ...
            lines.forEach(line => {
              ...
              if (parts.length >= 2) {
                ...
                const name = parts.find(p => p.length > 2 && /[a-zA-ZÀ-ỹ]/.test(p) && p !== mssv);
                if (mssv) {
                  studentsData.push({ mssv, name: name || `Sinh viên ${mssv}` });
                }
              }
            });
          }
          ...
          if (studentsData.length === 0) {
            alert('Không tìm thấy dữ liệu sinh viên hợp lệ. Vui lòng đảm bảo file có cột MSSV và Tên.');
            return;
          }

          const token = localStorage.getItem('token');
          const res = await fetch(getApiUrl(`/api/classes/${classData._id}/import-students`), {
            method: 'POST',
            headers: {
          ...
          {showAttendanceReport && (
            <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                ...
                <button onClick={() => setShowAttendanceReport(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#64748b' }}>Đóng x</button>
              </div>
              
              {loadingReport ? (
                <p>Đang tải dữ liệu...</p>
              ) : attendanceData.length === 0 ? (
                <p style={{ fontSize: '13px', color: '#64748b', fontStyle: 'italic' }}>Chưa có sinh viên nào trong danh sách lớp.</p>
              ) : (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid #e2e8f0', textAlign: 'left', color: '#64748b' }}>
                        ...
                        <th style={{ padding: '8px', textAlign: 'center' }}>MSSV</th>
                        <th style={{ padding: '8px', textAlign: 'center' }}>Điểm danh</th>
                        <th style={{ padding: '8px', textAlign: 'center' }}>Số lần Out</th>
                      </tr>
                    </thead>
                    <tbody>
                      {attendanceData.map((s, idx) => (
                        <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9' }}>
                          ...
                          <td style={{ padding: '8px', textAlign: 'center' }}>{s.mssv || '---'}</td>
                          <td style={{ padding: '8px', textAlign: 'center' }}>
                            {s.isPresent ? <span title="Đã điểm danh">✅</span> : <span title="Vắng">➖</span>}
                          </td>
                          <td style={{ padding: '8px', textAlign: 'center' }}>
                            <span style={{ 
                              color: s.leaveCount > 2 ? '#ef4444' : '#64748b',
                              fontWeight: s.leaveCount > 2 ? 'bold' : 'normal'
                            }}>
                              {s.leaveCount}
                            </span>
                          </td>
