  ...
  const handleImportStudents = async () => {
    ...
    input.onchange = async (e: any) => {
      ...
      reader.onload = async (event) => {
        ...
        try {
          // Lấy ID của lớp học đầu tiên hoặc lớp đang chọn (giả sử có 1 lớp cho demo)
          const classesRes = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/classes`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
          });
          const classes = await classesRes.json();
          
          if (classes.length === 0) {
            alert('Bạn chưa tạo lớp học nào.');
            return;
          }

          const classId = classes[0]._id; // Demo: lấy lớp đầu tiên

          const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/classes/${classId}/import-students`, {
            method: 'POST',
            headers: {
          ...
          <div className="modal-content large" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                <h4 style={{ marginBottom: '10px' }}>📤 Import danh sách cho lớp học</h4>
                <p style={{ fontSize: '13px', color: '#64748b', marginBottom: '15px' }}>
                  Vui lòng chọn file .txt hoặc .csv chứa danh sách email sinh viên để thêm vào lớp.
                </p>
                <div className="student-actions" style={{ justifyContent: 'flex-start' }}>
                  <button className="btn-primary" onClick={handleImportStudents}>📁 Chọn file và Import</button>
                </div>
              </div>
